...
export const API_BASE_URL = codespaceName ? `https://${codespaceName}-8000.app.github.dev` : ''

export async function fetchCollection(resource) {
  const normalizedResource = resource.startsWith('/api/') ? resource : `/api/${resource}`
  const response = await fetch(`${API_BASE_URL}${normalizedResource}/`)
